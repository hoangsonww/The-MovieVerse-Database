...
function populateMovieDetails(movie, imdbRating, rtRating, metascore, awards, rated) {
  ...
  document.getElementById('movie-description').innerHTML += `
        ...
        <p style="cursor: pointer" title="Your rating also counts - it might take a while for us to update!"><strong>Averaged User Ratings:</strong> <span id="userRatings">${scaledRating}/5.0 (based on <strong>${
          movie.vote_count
        }</strong> votes)</span></p>
        ${awardsElement}
        ${metascoreElement}
