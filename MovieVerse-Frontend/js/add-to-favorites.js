...
export async function checkAndUpdateFavoriteButton() {
  ...
  try {
    if (!userEmail) {
      console.log(
        "User is not signed in. Checking local storage for favorites.",
      );
    ...
    if (error.code === "resource-exhausted") {
      console.log(
        "Firebase quota exceeded. Checking local storage for favorites.",
      );
      const localFavorites =
        JSON.parse(localStorage.getItem("moviesFavorited")) || [];
      updateFavoriteButton(movieId, localFavorites);
...
export async function toggleFavorite() {
  ...
  try {
    ...
    if (querySnapshot.empty && userEmail === "") {
      ...
      console.log(
        "No user found with that email and user is supposed to be signed in.",
      );
      return;
    }
