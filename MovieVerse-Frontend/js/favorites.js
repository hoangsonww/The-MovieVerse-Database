...
document
  .getElementById("delete-watchlist-btn")
  .addEventListener("click", () => openModal("delete-watchlist-modal"));
...
async function populateCreateModalWithFavorites() {
  try {
    ...
    if (!currentUserEmail) {
      const moviesFavorited =
        JSON.parse(localStorage.getItem("moviesFavorited")) || [];
      const favoritesTVSeries =
        JSON.parse(localStorage.getItem("favoritesTVSeries")) || [];
      ...
      if (!container) {
        ...
        document
          .getElementById("create-watchlist-form")
          .insertBefore(
            container,
      ...
      moviesLabel.textContent =
        "Select favorite movies to include in watchlist:";
      ...
      if (moviesFavorited.length === 0) {
        moviesContainer.innerHTML =
          '<p style="margin-top: 20px; color: inherit;">No Favorite Movies Added Yet.</p>';
      } else {
      ...
      tvSeriesLabel.textContent =
        "Select favorite TV series to include in watchlist:";
      ...
      if (favoritesTVSeries.length === 0) {
        tvSeriesContainer.innerHTML =
          '<p style="margin-top: 20px; color: inherit;">No Favorite TV Series Added Yet.</p>';
      } else {
    ...
    if (!userSnapshot.empty) {
      ...
      container.innerHTML =
        '<p style="text-align: center; color: inherit;">No favorites found. Please add some favorites first.</p>';
    }
  } catch (error) {
...
document
  ...
  .addEventListener("submit", async function (e) {
    ...
    const description = document.getElementById(
      "new-watchlist-description",
    ).value;
    const selectedMovies = Array.from(
      document.querySelectorAll("#movies-container input:checked"),
    ).map((checkbox) => checkbox.value);
    const selectedTVSeries = Array.from(
      document.querySelectorAll("#tvseries-container input:checked"),
    ).map((checkbox) => checkbox.value);
    ...
    let maxOrder = 0;
    let localWatchlists =
      JSON.parse(localStorage.getItem("localWatchlists")) || [];
    ...
    try {
      if (currentUserEmail) {
        ...
        maxOrder = querySnapshot.docs.reduce(
          (max, docSnapshot) => Math.max(max, docSnapshot.data().order || 0),
          0,
        );
        isDuplicate = querySnapshot.docs.some(
          (doc) => doc.data().name.toLowerCase() === name.toLowerCase(),
        );
      } else {
        isDuplicate = localWatchlists.some(
      ...
      if (error.code === "resource-exhausted") {
        console.log(
          "Firebase quota exceeded. Using localStorage for watchlists.",
        );
        localWatchlists.push({
          id: `local-${new Date().getTime()}`,
          userEmail: "",
...
document
  .getElementById("create-watchlist-btn")
  .addEventListener("click", function () {
...
document
  .getElementById("edit-watchlist-btn")
...
async function populateEditModal() {
  try {
    let currentUserEmail = localStorage.getItem(
      "currentlySignedInMovieVerseUser",
    );
    ...
    select.addEventListener("change", function () {
      const selectedWatchlist = watchlists.find(
        (watchlist) => watchlist.id === this.value,
      );
...
document
  .getElementById("edit-watchlist-form")
  .addEventListener("submit", async function (e) {
    try {
      showSpinner();
      e.preventDefault();
      ...
      const currentMoviesSelection = Array.from(
        document.querySelectorAll(
          '#edit-movies-container input[type="checkbox"]:checked',
        ),
      ).map((checkbox) => checkbox.value);
      const currentTVSeriesSelection = Array.from(
        document.querySelectorAll(
          '#edit-tvseries-container input[type="checkbox"]:checked',
        ),
      ).map((checkbox) => checkbox.value);

      const moviesSelectionChanged = !(
        initialMoviesSelection.length === currentMoviesSelection.length &&
        initialMoviesSelection.every((value) =>
          currentMoviesSelection.includes(value),
        )
      );
      const tvSeriesSelectionChanged = !(
        initialTVSeriesSelection.length === currentTVSeriesSelection.length &&
        initialTVSeriesSelection.every((value) =>
          currentTVSeriesSelection.includes(value),
        )
      );
      ...
      if (currentUserEmail) {
        ...
        let localWatchlists =
          JSON.parse(localStorage.getItem("localWatchlists")) || [];
        let watchlistIndex = localWatchlists.findIndex(
...
async function populateDeleteModal() {
  try {
    ...
    if (watchlists.length === 0) {
      deleteForm.innerHTML =
        '<p style="margin-top: 20px">No Watchlists Available to Delete.</p>';
      return;
    }
...
async function deleteSelectedWatchlists() {
  try {
    ...
    const selectedCheckboxes = document.querySelectorAll(
      '#delete-watchlist-checkboxes-container input[type="checkbox"]:checked',
    );
    ...
    if (currentUserEmail) {
      ...
      watchlists = watchlists.filter(
        (watchlist) => !selectedIds.includes(watchlist.id),
      );
...
document
  .getElementById("delete-watchlist-btn")
  .addEventListener("click", populateDeleteModal);
...
async function fetchMovieDetails(movieId) {
  ...
  try {
    ...
    errorDiv.textContent =
      "Error loading movie card. Please try refreshing the page.";
    return errorDiv;
  }
}
...
async function createMovieCard(movie) {
  ...
  const ratingClass =
    movie.vote_count === 0 ? "unrated" : getClassByRate(movie.vote_average);
  const voteAvg =
    movie.vote_count === 0 ? "Unrated" : movie.vote_average.toFixed(1);
  ...
  try {
    ...
    if (currentUserEmail) {
      try {
        ...
        localStorage.setItem(
          "cachedWatchlists_" + currentUserEmail,
          JSON.stringify(watchlists),
        );
      } catch (firebaseError) {
        console.warn(
          "Firebase fetch failed, loading from cache:",
          firebaseError,
        );
        ...
        const cachedWatchlists =
          JSON.parse(
            localStorage.getItem("cachedWatchlists_" + currentUserEmail),
          ) || [];
      ...
      if (watchlists.length === 0) {
        displaySection.innerHTML =
          '<p style="text-align: center">No watch lists found. Click on "Create Watch Lists" to start adding movies.</p>';
      } else {
        displaySection.innerHTML = "";
        displaySection.innerHTML +=
        ...
        document
          .getElementById("watchlist-header")
      ...
      if (localWatchlists.length === 0) {
        displaySection.innerHTML =
          '<p style="text-align: center">No watch lists found. Start by adding movies to your watchlist.</p>';
      } else {
        displaySection.innerHTML = "";
        displaySection.innerHTML +=
          '<p style="text-align: center; margin-top: 20px; color: white"><strong>Your Watch Lists</strong></p>';
        localWatchlists.sort((a, b) =>
          b.pinned === a.pinned ? 0 : b.pinned ? 1 : -1,
        );
    ...
    if (currentUserEmail) {
      try {
        ...
        if (!userSnapshot.empty) {
          ...
          // Cache the results if successful
          localStorage.setItem(
            "cachedFavorites_" + currentUserEmail,
            JSON.stringify({ favorites, favoritesTVSeries }),
          );
        }
      } catch (firebaseError) {
        console.warn(
          "Firebase favorites fetch failed, loading from cache:",
          firebaseError,
        );
        ...
        const cachedFavorites =
          JSON.parse(
            localStorage.getItem("cachedFavorites_" + currentUserEmail),
          ) || {};
...
async function fetchTVSeriesDetails(tvSeriesId) {
  ...
  try {
    ...
    const errorDiv = document.createElement("div");
    errorDiv.textContent =
      "Error loading series details. Please try refreshing the page.";
    return errorDiv;
  }
}
...
async function getAdditionalTVSeriesImages(tvSeriesId) {
  const response = await fetch(
    `https://api.themoviedb.org/3/tv/${tvSeriesId}/images?api_key=${getMovieCode()}`,
  );
  const data = await response.json();
  return data.posters.map((poster) => poster.file_path);
}
...
async function createTVSeriesCard(tvSeries) {
  ...
  const ratingClass =
    tvSeries.vote_count === 0
      ? "unrated"
      : getClassByRate(tvSeries.vote_average);
  const voteAvg =
    tvSeries.vote_count === 0 ? "Unrated" : tvSeries.vote_average.toFixed(1);
...
function shareWatchList(watchlistDiv) {
  const watchlistTitle =
    watchlistDiv.querySelector(".watchlist-title").textContent;
...
function createWatchListDiv(watchlist) {
  ...
  if (watchlist.movies === undefined) {
    ...
    watchlist.movies.forEach((movieId) => {
      fetchMovieDetails(movieId).then((movieCard) =>
  ...
  if (watchlist.tvSeries === undefined) {
    ...
    watchlist.tvSeries.forEach((tvSeriesId) => {
      fetchTVSeriesDetails(tvSeriesId).then((tvSeriesCard) =>
...
function updateWatchlistsOrderInLS() {
  const watchlistsDivs = document.querySelectorAll(
    "#watchlists-display-section > .watchlist",
  );
  let watchlists = JSON.parse(localStorage.getItem("localWatchlists")) || [];
  const newOrder = Array.from(watchlistsDivs).map((div) =>
    div.getAttribute("data-watchlist-id"),
  );
  ...
  if (currentUserEmail) {
    try {
      ...
      const index = watchlists.findIndex(
        (watchlist) => watchlist.docId === watchlistId,
      );
    ...
    const sibling = moveUp
      ? watchlistDiv.previousElementSibling
      : watchlistDiv.nextElementSibling;
    if (sibling) {
...
function updateWatchlistsCreated() {
  let watchlistsCount =
    parseInt(localStorage.getItem("watchlistsCreated")) || 0;
  watchlistsCount++;
