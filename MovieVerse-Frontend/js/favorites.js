...
async function populateCreateModalWithFavorites() {
  try {
    ...
    if (!currentUserEmail) {
      ...
      if (moviesFavorited.length === 0) {
        moviesContainer.innerHTML =
          '<p style="margin-top: 20px">No Favorite Movies Added Yet.</p>';
      } else {
      ...
      if (favoritesTVSeries.length === 0) {
        tvSeriesContainer.innerHTML =
          '<p style="margin-top: 20px">No Favorite TV Series Added Yet.</p>';
      } else {
    ...
    if (!userSnapshot.empty) {
      ...
      container.innerHTML =
        '<p style="text-align: center">No favorites found. Please add some favorites first.</p>';
    }
  } catch (error) {
