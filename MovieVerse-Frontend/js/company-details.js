...
async function fetchCompanyDetails(companyId) {
  ...
  try {
    ...
    const fullCountryName = twoLetterCountryCodes.find(
      (country) => country.code === company.origin_country,
    )?.name;

    document.getElementById("company-name").textContent =
      company.name || "Information Unavailable";
    document.getElementById("company-headquarters").textContent =
      company.headquarters || "Information Unavailable";
    document.getElementById("company-country").textContent =
      fullCountryName || "Information Unavailable";
...
async function fetchCompanyMovies(companyId) {
  ...
  try {
    ...
    if (data.results.length === 0) {
      const companyMoviesContainer = document.getElementById(
        "company-movies-container",
      );
