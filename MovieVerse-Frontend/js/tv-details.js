...
async function populateTvSeriesDetails(tvSeries, imdbRating) {
  ...
  const networks =
    tvSeries.networks && tvSeries.networks.length
      ? tvSeries.networks.map((network) => network.name).join(", ")
  ...
  const voteAverage = tvSeries.vote_average
    ...
    : "N/A";
  const voteCount = tvSeries.vote_count
  ...
  detailsHTML += `<p title="Your rating also counts - it might take a while for us to update!"><strong>MovieVerse User Rating:</strong> <strong id="user-ratings">${(
    ...
    1,
  )}/5.0</strong> (based on <strong id="user-ratings">${voteCount}</strong> votes)</p>`;
  ...
  let tmdbRating = tvSeries.vote_average
    ? tvSeries.vote_average.toFixed(1)
    : "N/A";
  if (tmdbRating === "N/A") {
  ...
  const homepage = tvSeries.homepage
    ? `<a id="homepage" href="${tvSeries.homepage}" target="_blank">Visit homepage</a>`
    : "Not available";
  detailsHTML += `<p><strong>Homepage:</strong> ${homepage}</p>`;

  if (tvSeries.origin_country && tvSeries.origin_country.length > 0) {
    const countryNames = tvSeries.origin_country
  ...
  const productionCountries =
    tvSeries.production_countries && tvSeries.production_countries.length > 0
      ? tvSeries.production_countries
          .map((country) => getCountryName(country.iso_3166_1))
          .join(", ")
      : "Information not available";
  ...
  if (
    ...
    let keywordsHTML = tvSeries.keywords.results
