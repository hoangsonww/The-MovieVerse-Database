...
async function fetchTvDetails(tvSeriesId) {
  ...
  try {
    ...
    const timelinePromise = fetchSeasonsTimeline(tvSeriesId, tvSeriesDetails.name || tvSeriesDetails.original_name);

    // Fetch IMDB rating in background and update when ready
    if (imdbId) {
      fetchTVRatings(imdbId)
        .then(data => {
          const imdbRating = data !== 'IMDb rating' && data?.imdbRating ? data.imdbRating : 'View on IMDb';
...
async function populateTvSeriesDetails(tvSeries, imdbRating, rated) {
  ...
  let detailsHTML = `
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 20px; margin: 30px 0;">
      ...
      <!-- Keywords Card -->
      ${
        tvSeries.keywords && tvSeries.keywords.results && tvSeries.keywords.results.length > 0
          ? `
      <div style="background: linear-gradient(135deg, rgba(255,152,0,0.1) 0%, rgba(255,152,0,0.05) 100%); backdrop-filter: blur(10px); border-radius: 20px; padding: 20px; border: 1px solid rgba(255,152,0,0.3);">
...
function displayTVStatsDashboard(tvSeries) {
  ...
  const popularity = Math.min(100, Math.round((tvSeries.popularity / 1000) * 100));
...
function displaySeriesOverview(tvSeries) {
  ...
  if (seasonsBar && episodesBar) {
    ...
    setTimeout(() => {
      ...
      if (totalRuntime && tvSeries.episode_run_time && tvSeries.episode_run_time.length > 0) {
...
function displayTVAdditionalStats(tvSeries) {
  ...
  if (firstAirEl && tvSeries.first_air_date) {
    ...
    firstAirEl.textContent = date.toLocaleDateString('en-US', { year: 'numeric', month: 'short' });
  }
  ...
  if (lastAirEl && tvSeries.last_air_date) {
    ...
    lastAirEl.textContent = date.toLocaleDateString('en-US', { year: 'numeric', month: 'short' });
  }
...
function displaySeasonsTimeline(seasons, tvSeriesId, seriesName) {
  ...
  seasons.forEach((season, index) => {
    ...
    seasonLabel.textContent = season.season_number === 0 ? 'Specials' : `Season ${season.season_number}`;
